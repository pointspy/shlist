...
open class TinyConsole {
    ...
    /**
        Print to tiny without **time**.
         - Don't print to xcode log by **default**.
         - Parameter text: content
         - Parameter global: **true** if print to system log
     */
    
    public static func printWithoutTime(_ text: NSAttributedString, global: Bool = false) {
        // When we leave this method and global is true, we want to print it to console
        defer {
            if global {
                Swift.print(text.string)
        ...
        DispatchQueue.main.async {
            let timeStamped = NSMutableAttributedString(string: "")
            let range = NSRange(location: 0, length: timeStamped.length)
